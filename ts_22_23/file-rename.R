...
list.txt <- list.files(path = "Data_2023/Fichiers texte/", recursive = TRUE,
           pattern = "\\.txt$", 
           full.names = TRUE)
list.txt
...
new.txt. <- paste0("Data_2023/Fichiers texte renamed/",ini.name,"_",end.name,"_",substr(txt.,25,nchar(txt.)))

file.copy(
  from=txt., 
  to=paste0("Data_2023/Fichiers texte renamed/",ini.name,"_",end.name,"_",substr(txt.,26,nchar(txt.)))
  )
