...
library(tidyr)
library(xlsx)
setwd("D:/PNMCCA/SAMBAT Suivis sonde NKE")
```
...
list_of_files <- list.files(path = "D:/PNMCCA/SAMBAT Suivis sonde NKE/Data/Fichiers textes/", recursive = TRUE,
                            pattern = "\\.txt$", 
                            full.names = TRUE)
...
```{r}
start <- read_delim("Data/Fichiers csv/all begining corrected.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE, col_names = FALSE,
    skip = 0)
start<- start[-1,]

middle <- read_delim("Data/Fichiers csv/17-02-2022_21-07-2022b.csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE, col_names = FALSE,
    skip = 0)
middle<- middle[-1,]
...
colnames <- read_delim("Data/Fichiers csv/17-02-2022_21-07-2022b.csv", 
...
saveRDS(brut, "Données brutes.rds")
...
saveRDS(cor, "Données corrigées et nettoyées.rds")
...
table(yes$mois)
library(readxl)
pluie <- read_excel("Data/Precipitation.xlsx")
...
ggplot(pluie, aes(mois, Pluie, col = Site)) +
  geom_point()+
  xlab("\nDate") +
  ylab("")+
...
m2 <- melt(all[, c(2:6,10, 11, 12)], id.vars= "mois")
m2 <- m2 %>% drop_na(mois)
m2$mois <- as.factor(m2$mois)
m2$value <- as.numeric(m2$value)

View(m2)
ggplot(m2, aes(mois, value)) +
  geom_boxplot()+
  xlab("\nDate") +
  ylab("")+
  facet_grid(variable~., scales = "free",  
...
ggsave("Time series - 18 août en rouge.png")
...
ggsave("Temperature - 18 août en rouge.png")
...
ggsave("Time series du 1er aout au 1 sept - 18 août en rouge et 14 aout en bleu.png")
```

Zoom sur mars 2022 fluorescence et oxygène

```{r}
focus2 <- dplyr::filter(yes, date > "2022-03-01" & date < "2022-03-31")
focus2 <- melt(focus2[, c(4,6,8)], id.vars= "Datetime")
focus$value <- as.numeric(focus$value)
xstart2 <- as.POSIXct("2022-08-13 23:00:00")
xend2 <- as.POSIXct("2022-08-15 01:00:00")

ggplot(focus2, aes(Datetime, value)) +
  geom_point(size = 1)+
    #scale_x_datetime(breaks = "1 month", date_labels = "%b") +
  xlab("\nDate") +
  ylab("°C")+
  theme_bw() +
  facet_grid(variable~., scales = "free",  
             labeller = as_labeller(c(Oxygene= bquote('Oxygène dissous \n (mgl)'),
                                      Fluorescence= "Fluorescence \n (µgl)")))
