...
saveRDS(df, "ts_24/raw.data_2024.rds")
...
# Anlayse data via ploting when difftime != 15 min
...
bluelines.holes <- df[c(sort(unique(c(z,ind)))),]$Datetime 
...
ts_22to24.raw <- rbind(ts_22to23.raw, df.raw[,1:10])
ts_22to24.clean <- rbind(ts_22to23.clean, df.clean[,1:10])
...
go1 <- melt(ts_22to24.raw[, c(2:6,8)], id.vars= "Datetime")
go1$value <- as.numeric(go1$value)
...
buoy.plot <- function(df., var., ylab.) {
  ...
  p. <<- ggplot(df.plot, aes(Datetime, plot.var.)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust=1))
  # 2024 and more
  for(i in 1:length(bluelines.holes)){ 
    p. <<- p. + geom_vline(aes(xintercept = bluelines.holes[i]), col = "blue", alpha = 0.4)
  }
  #return(p.)
}

buoy.plot(df. = ts_22to24.raw, var. = "Temperature", ylab. = "Température (°C)")
(p.temp.raw <- p.)
rm(p.)
ggsave("ts_24/Figs./raw/p.temp.raw.png", width = 5, height = 2.5, dpi = 300)

buoy.plot(df. = ts_22to24.raw, var. = "Salinite", ylab. = "Salinité")
(p.sal.raw <- p.)
rm(p.)
ggsave("ts_24/Figs./raw/p.sal.raw.png", width = 5, height = 2.5, dpi = 300)

buoy.plot(df. = ts_22to24.raw, var. = "Oxygene", ylab. = "Oxygène dissous (mg/L)")
(p.oxy.raw <- p.)
rm(p.)
ggsave("ts_24/Figs./raw/p.oxy.raw.png", width = 5, height = 2.5, dpi = 300)

buoy.plot(df. = ts_22to24.raw, var. = "pH", ylab. = "pH")
(p.pH.raw <- p.)
rm(p.)
ggsave("ts_24/Figs./raw/p.pH.raw.png", width = 5, height = 2.5, dpi = 300)

buoy.plot(df. = ts_22to24.raw, var. = "Fluorescence", ylab. = "Chla (µg/L)")
(p.fluo.raw <- p.)
rm(p.)
...
go1 <- melt(ts_22to24.clean[, c(2:6,8)], id.vars= "Datetime")
go1$value <- as.numeric(go1$value)
...
buoy.plot.clean(df. = ts_22to24.clean, var. = "Temperature", ylab. = "Température (°C)")
(p.temp.clean <- p.)
rm(p.)
ggsave("ts_24/Figs/clean/p.temp.clean.png", width = 5, height = 2.5, dpi = 300)

buoy.plot.clean(df. = ts_22to24.clean, var. = "Salinite", ylab. = "Salinité")
(p.sal.clean <- p.)
rm(p.)
ggsave("ts_24/Figs/clean/p.sal.clean.png", width = 5, height = 2.5, dpi = 300)

buoy.plot.clean(df. = ts_22to24.clean, var. = "Oxygene", ylab. = "Oxygène dissous (mg/L)")
(p.oxy.clean <- p.)
rm(p.)
ggsave("ts_24/Figs/clean/p.oxy.clean.png", width = 5, height = 2.5, dpi = 300)

buoy.plot.clean(df. = ts_22to24.clean, var. = "pH", ylab. = "pH")
(p.pH.clean <- p.)
rm(p.)
ggsave("ts_24/Figs/clean/p.pH.clean.png", width = 5, height = 2.5, dpi = 300)

buoy.plot.clean(df. = ts_22to24.clean, var. = "Fluorescence", ylab. = "Chla (µg/L)")
(p.fluo.clean <- p.)
rm(p.)
...
diff.ts <- setdiff(ts_22to24.raw,ts_22to24.clean)
diff.temp.ts <- setdiff(ts_22to24.raw[,c(2,8:10)],ts_22to24.clean[,c(2,8:10)])
diff.sal.ts <- setdiff(ts_22to24.raw[,c(3,8:10)],ts_22to24.clean[,c(3,8:10)])
diff.O2.ts <- setdiff(ts_22to24.raw[,c(4,8:10)],ts_22to24.clean[,c(4,8:10)])
diff.pH.ts <- setdiff(ts_22to24.raw[,c(5,8:10)],ts_22to24.clean[,c(5,8:10)])
diff.fluo.ts <- setdiff(ts_22to24.raw[,c(6,8:10)],ts_22to24.clean[,c(6,8:10)])

p <- ggplot(ts_22to24.raw, aes(x=Datetime, y=Temperature)) + 
  geom_point() +
  xlab("") +
  ylab("Température (°C)")
p.temp <- p + geom_point(data=diff.temp.ts,colour="red")

p <- ggplot(ts_22to24.raw, aes(x=Datetime, y=Salinite)) + 
  geom_point() +
  xlab("") +
  ylab("Salinité")
p.sal <- p + geom_point(data=diff.sal.ts,colour="red")

p <- ggplot(ts_22to24.raw, aes(x=Datetime, y=Oxygene)) + 
  geom_point() +
  xlab("") +
  ylab("Oxygène dissous (mg/L)")
p.O2 <- p + geom_point(data=diff.O2.ts,colour="red")

p <- ggplot(ts_22to24.raw, aes(x=Datetime, y=pH)) + 
  geom_point() +
  xlab("") +
  ylab("pH (pH)")
p.pH <- p + geom_point(data=diff.pH.ts,colour="red")

p <- ggplot(ts_22to24.raw, aes(x=Datetime, y=Fluorescence)) + 
  geom_point() +
  xlab("") +
  ylab("Chla (µg/L)")
...
y. <- c(min(ts_22to24.raw$Temperature, na.rm=T), max(ts_22to24.raw$Temperature, na.rm=T))
buoy.plot.del.(df. = ts_22to24.raw, var. = "Temperature", ylab. = "Température (°C)", ylim. = y.)
...
y. <- c(25, max(ts_22to24.raw$Salinite, na.rm=T))
buoy.plot.del.(df. = ts_22to24.raw, var. = "Salinite", ylab. = "Salinite", ylim. = y.)
...
y. <- c(min(ts_22to24.raw$Oxygene, na.rm=T), max(ts_22to24.raw$Oxygene, na.rm=T))
buoy.plot.del.(df. = ts_22to24.raw, var. = "Oxygene", ylab. = "Oxygène dissous (mg/L)", ylim. = y.)
...
y. <- c(5,9)
buoy.plot.del.(df. = ts_22to24.raw, var. = "pH", ylab. = "pH", ylim. = y.)
...
y. <- c(min(ts_22to24.raw$Fluorescence, na.rm=T), 10)
buoy.plot.del.(df. = ts_22to24.raw, var. = "Fluorescence", ylab. = "Chla (µg/L)", ylim. = y.)
...
m <- melt(ts_22to24.clean[, c(2:6,10)], id.vars= "mois")
m <- m %>% drop_na(mois)
